...
it("checks every push destination, including feature-to-dev and deletions", () => {
  ...
  for (const branch of ["dev", "staging", "main"]) {
    expect(run(`refs/heads/codex/example a refs/heads/${branch} b\n`).status).toBe(1);
    expect(run(`(delete) 0000 refs/heads/${branch} b\n`).status).toBe(1);
  }
  expect(run("refs/heads/codex/example a refs/heads/codex/example b\n").status).toBe(0);
  expect(run("refs/heads/codex/example a refs/heads/codex/example b\nrefs/heads/codex/second a refs/heads/dev b\n").status).toBe(1);
});
...
it("allows a task rebase to replay commits in detached HEAD", () => {
  ...
  git("add", "task.txt");
  git("commit", "-qm", "task");
  git("switch", "-qc", "codex/upstream", "dev");
  writeFileSync(join(repo, "upstream.txt"), "upstream work\n");
  git("add", "upstream.txt");
  git("commit", "-qm", "upstream");
  git("rebase", "codex/upstream", "codex/task");
  expect(git("branch", "--show-current").trim()).toBe("codex/task");
  expect(git("merge-base", "--is-ancestor", "codex/upstream", "HEAD")).toBe("");
...
it("reports WIP even when HEAD equals dev, without modifying any worktree", () => {
  ...
  git("worktree", "add", "-qb", "codex/pending", worktree);
  ...
  expect(output).toContain("codex/pending: 4 arquivo(s) pendente(s), 0 commit(s) fora de origin/dev");

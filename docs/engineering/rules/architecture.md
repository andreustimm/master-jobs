...
`setApplicationStatus` (contexto `pursuit`), e o desfazer
(`undoApplicationStatus`) grava pelo mesmo `commitOverSnapshot`; a UI e a CLI
(`jho track` e `jho jobs add --status`) são chamadores dele, e uma mudança de status feita no navegador cai em

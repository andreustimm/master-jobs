...
| operations | pedido de manutenção do acervo: qual rotina rodar e quem executa | nenhuma tabela própria — o estado mora nas tabelas que cada rotina escreve | `src/contexts/operations/index.ts` | `WorkflowDispatchPort` (GitHub Actions hoje); lê a saúde das fontes por `src/core/ingest/health.ts` |

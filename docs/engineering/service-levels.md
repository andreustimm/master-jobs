...
| Latência das jornadas `/jobs`, `/` e `/searches` | **95% em até 2 s**, por rota | Duração de servidor de todas as requisições elegíveis, incluindo falhas; mínimo de 100 amostras por rota | **Sem dados suficientes**; o tracing do Sentry ([#219](https://github.com/andreustimm/master-jobs/issues/219)) amostra 10% das requisições a partir do deploy que o inclui, e as 100 amostras por rota ainda precisam acumular. O trace sozinho é **enviesado**: a requisição morta por `FUNCTION_INVOCATION_TIMEOUT` nunca envia transação, então o p95 do Sentry omite justamente os eventos ruins mais graves — os timeouts entram pelo log da Vercel ([operations.md](../operations.md)) |

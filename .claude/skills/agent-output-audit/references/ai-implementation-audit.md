...
For each red flag, run the listed scan against the test diff since the task baseline. Use `rtk git log --follow <test_file>` to recover the baseline. Emit the listed verdict when the flag fires.

### RF-1 Skipped or disabled tests added

```bash
rtk git diff <baseline_sha>..HEAD -- '*test*' '*spec*' \
  | rtk rg -nP '^\+.*(\.skip\(|\.only\(|xit\(|xdescribe\(|t\.Skip\(|@pytest\.mark\.skip|@Ignore|fdescribe|fit\()'
```
...
```bash
rtk git diff <baseline_sha>..HEAD -- '*test*' '*spec*' | rtk rg -nP '^-.*(toBe|toEqual|toStrictEqual)\('
rtk git diff <baseline_sha>..HEAD -- '*test*' '*spec*' | rtk rg -nP '^\+.*(toBeDefined|toBeTruthy|toBeFalsy|toBeNull|toBeUndefined|toMatch\b|expect\.anything|expect\.any\b|assert\.NotNil|require\.NotEmpty)'
```
...
```bash
rtk rg -nP '(jest\.mock\(|vi\.mock\(|nock\(|gomock\.|httpmock|patch\.object\(|patch\(.*Mock|sinon\.stub\(|MockBean\b)' <test_file>
```
...
```bash
rtk git diff --name-only <baseline_sha>..HEAD \
  | rtk rg -nP '(__snapshots__/.*\.snap$|testdata/golden/|/__fixtures__/|\.golden$|/fixtures/.*\.(json|yaml|yml)$)'
```
...
```bash
rtk git log --format='commit %H %s' --name-only <baseline_sha>..HEAD
```

Group the complete path list by `commit` header. Flag groups containing both an
implementation path and its test sibling without a commit message that names
the requirement. Do not filter to test-shaped paths before this comparison.
Then apply the Requirement → Test Mapping below.

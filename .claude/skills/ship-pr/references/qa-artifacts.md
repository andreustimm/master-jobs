...
In master-jobs the durable layout is `docs/qa/`: reports are in `reports/`,
bugs in `bugs/`, charters in `charters/`, and evidence is deliberately ignored
under `evidence/`. Use the newest report relevant to the branch and link only
versioned artifacts; never claim an ignored local screenshot will render on
GitHub. The Compozy convention below remains valid for task-local artifacts.
...
rtk test -d "$QA_DIR/screenshots" && rtk ls "$QA_DIR/screenshots"
rtk test -f "$QA_DIR/verification-report.md" && rtk head -40 "$QA_DIR/verification-report.md"
rtk ls "$QA_DIR/issues" 2>/dev/null | rtk wc -l
```

...
```bash
# CommonJS
rtk node -e 'const c=require("./commitlint.config.cjs"); console.log(JSON.stringify(c.rules,null,2))'

# ESM (.mjs, or .js under type=module)
rtk node --input-type=module -e 'const c=(await import("./commitlint.config.mjs")).default; console.log(JSON.stringify(c.rules,null,2))'

# TypeScript on this project's Node 24 runtime
rtk node --experimental-strip-types --input-type=module -e 'const c=(await import("./commitlint.config.ts")).default; console.log(JSON.stringify(c.rules,null,2))'

# JSON
rtk jq '.rules // ."commitlint" // .extends' .commitlintrc.json

# package.json
rtk jq '.commitlint.rules' package.json
```
...
```bash
rtk git commit -m "feat(auth): allow multiple active workspaces per session" \
  -m "Adds a workspace-switcher in the navbar and a new GET /v1/accounts endpoint listing accessible workspaces."
```
...
- Scope only when the repo uses scopes consistently. Inspect the last 20 commits with `rtk git log --oneline -20` to learn the convention.
...
```bash
rtk git add path/to/file-a.ts path/to/file-b.ts path/to/docs.md
```

Never use `rtk git add -A`, `rtk git add .`, or `rtk git add -u`:

- They silently include unrelated files in `rtk git status` (someone else's WIP, generated `.env`, build artifacts).
...
If `rtk git commit` fails inside a hook (commitlint, lint-staged, format-staged, prettier, etc.):

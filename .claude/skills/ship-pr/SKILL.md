...
Local repository rules are part of this skill's hard preflight: work only in a
task worktree created from `dev`, prefix shell invocations with `rtk`, run
`deep-review` and resolve every blocking verdict before shipping, and create the
...
- **Core path (always runs):** explore impact → build PR description → commit → `rtk gh pr create`.
...
```bash
rtk bash .claude/skills/ship-pr/scripts/detect-tooling.sh
```
...
Hard requirements: `gh` on PATH and a non-protected feature branch containing
commits ahead of `dev`, working-tree changes, or both. A clean branch ahead of
`dev` is ready to ship; abort only when neither commits nor changes exist.
...
### 4. (Optional) Skeeper sync

Only if the detector reports `skeeper_configured: true` (i.e., `.skeeper/` directory and `skeeper.lock` both exist):

```bash
rtk skeeper sync --dry-run         # preview
rtk skeeper sync --commit -m "..."  # apply + stage skeeper.lock
```
...
Detect the repo's commit convention via the detector's `commitlint_config` value (fallback to `@commitlint/config-conventional` defaults). Stage files **by name** (never `rtk git add -A`), pass the commit message directly with `rtk git commit -m`, and never use `--no-verify` or `--no-gpg-sign`.

If the feature branch is clean and already ahead of `dev`, skip this step; its
existing commits are the payload. Never create an empty shipping commit.
...
Refuse to push from a protected branch — opening a PR requires a feature branch with a distinct head:

```bash
BRANCH="$(rtk git rev-parse --abbrev-ref HEAD)"
case "$BRANCH" in
dev|staging|main|master|develop|trunk) echo "Refusing to ship from $BRANCH. Create a feature branch first." >&2; exit 1 ;;
esac
...
### 8. (Optional) CodeRabbit review-watch loop
...
2. Otherwise, derive a kebab-case slug from the branch name and create `.compozy/tasks/<slug>/` with `rtk mkdir -p` — the `compozy reviews watch` daemon populates the rest.

Then run, substituting `<slug>` and `<number>`:

```bash
rtk compozy reviews watch \

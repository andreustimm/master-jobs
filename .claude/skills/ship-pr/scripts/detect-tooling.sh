...
if [ -z "$commitlint_config" ] && [ -f package.json ]; then
    if rtk node -e 'const p = require("./package.json"); process.exit(Object.hasOwn(p, "commitlint") ? 0 : 1)' 2>/dev/null; then
        commitlint_config="package.json#commitlint"
    fi
fi

# QA artifact paths. Honor manual override first, otherwise include the living
# project tree and any task-local Compozy trees.
qa_paths_json="[]"
if [ -n "${QA_OUTPUT_PATH:-}" ] && [ -d "$QA_OUTPUT_PATH" ]; then
    qa_paths_json="[$(json_str "$QA_OUTPUT_PATH")]"
else
    qa_found=()
    if [ -d docs/qa/reports ] && find docs/qa/reports -type f -name '*.md' -print -quit | grep -q .; then
        qa_found+=("docs/qa")
    fi

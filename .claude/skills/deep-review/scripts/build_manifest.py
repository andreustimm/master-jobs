...
def untracked_stat(repo_root: Path, path: str):
    # PATCH LOCAL (ver .claude/skills/deep-review/PATCHES.md).
    #
    # `git ls-files --others` lista um symlink para diretório como UMA entrada, e
    # `read_bytes()` nela estoura com IsADirectoryError, derrubando o `--worktree`
    # inteiro. Não é caso raro aqui: este repositório usa symlink para paridade
    # de harness — `.codex/skills` e `.opencode/skills` apontam para
    # `.claude/skills` —, e enquanto não estiverem rastreados, aparecem aqui.
    #
    # Tratado como o binário já era: sem contagem de linha e não revisável.
    # `is_file()` segue o symlink, então symlink para ARQUIVO continua revisável.
    alvo = repo_root / path
    if not alvo.is_file():
        return None, None

...
Use `docs/qa` as the QA root. The browser driver is the pinned project
dependency, so every bare `agent-browser` command in the references means
`rtk pnpm exec agent-browser`. Install Chrome once with
...
- **qa-docs-path** (optional): root of the living QA docs tree; defaults to `docs/qa/`. Resolve it inside the repository and stop with an error if it escapes the repository. The tree is this skill's memory and the only location for its QA artifacts — never a temp dir. Governed product fixes and regression tests from Step 7 remain in their normal repository locations. If the tree doesn't exist, run `qa-report` first; it owns the tree and its bootstrap.
...
- Preconditions: the automated suite is green (a precondition, not a QA step — gaps route to `agent-output-audit`) and at least one in-scope public surface is reachable in a production-parity build (real server, real auth, no mocks). If no in-scope surface is reachable, name the exact gap and stop; partial browser/CLI/tool unavailability becomes blocked legs after the report is created, while reachable legs continue.
...
- **Done when:** every charter is walked to a recorded verdict, evidence captured at checkpoints and divergences, the debrief written to the report's Session Debriefs section, and all of that charter's scenario rows updated.
...
- **A flow hangs:** close the session, record it, retry once from a clean session, then mark it `Fail` and link the filed bug. Use a Blocked status only when its named human prerequisite or decision actually applies. A stall is a finding to file, never a thing to nudge past (`references/persona-fidelity.md`).
- **Credentials or test data missing:** use `Blocked (needs human verify)` only
  when a named human must provide the prerequisite; otherwise treat missing
  deterministic test setup as `Fail`. Record the exact prerequisite and proceed

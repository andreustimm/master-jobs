...
Smoke the changed surface across a bounded matrix. Start with three combined
profiles — desktop Chrome at 1280/light, desktop Firefox at 768/dark, and mobile
Safari or its closest available WebKit profile at 375/system theme. Add Chrome
on Android for mobile-specific changes, both motion settings for animation
changes, and any browser named by a bug or support signal. Record unavailable
profiles as parity gaps instead of implying the Cartesian product was run.

The dimensions and change triggers that drive those combined profiles are:

| Layer | Baseline / addition trigger |
|---|---|
| Browser | Latest Chrome + Safari + Firefox |
| Mobile | Safari on iPhone baseline; add Chrome on Android for mobile-specific changes |
| Viewport | 1280, 768, 375 |
| OS dark mode | Light AND dark |
| Reduced motion | Add On + Off when motion or transition behavior changed |
...
- [ ] Third-party-cookie policy matches the persona's real browser default; when auth depends on cross-site cookies, also run the blocked-cookie edge probe.

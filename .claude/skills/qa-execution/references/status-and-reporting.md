...
Every matrix row (one charter-scenario pair within one session) carries exactly one of seven values:

| Status | Meaning | Terminal? |
|---|---|---|
| `Pending` | Not yet walked in this run | no |
| `Pass` | Walked; expected observable confirmed per the evidence standard | yes |
| `Fail` | Walked; expected observable not met and the linked bug remains open | yes |
...
Mapping to tracker enums (the qa-report state-schema reference, routed at Step 8 of the SKILL): `Pass` → `qa_status: pass`; `Fail` → `fail` + `fix_status: pending`; `Fixed` → `pass` + `fix_status: fixed` + `retest_status: pass`; `Skipped` → `skipped`; the two Blocked → `blocked-verify` / `blocked-decision`.

## Report lifecycle

One report per run at `<qa-docs-path>/reports/<YYYY-MM-DDTHHMMSSffffffZ>-<nonce8>-<scope>.md`. Generate the UTC start instant at microsecond precision plus a random eight-hex nonce once; `scope` is the branch slug or cycle name, lowercased with non-alphanumerics collapsed to `-`. The complete stem is the run id and also names its evidence directory. Template: `<qa-docs-path>/templates/report.md`, falling back to `assets/report-template.md`.
...
- **Never overwritten across runs.** A new run always gets a fresh UTC run id, even for the same scope on the same day. Only an explicit resume reopens an existing report.
...
Verdict order is deterministic: a red exit gate, an open Blocks-Completion /
Data-Loss finding, or several open Trust-Damage findings on the same journey is
`not-ready`; otherwise any blocked row is `ready-with-blocked-items` only when
its exact human prerequisite is disclosed and no unblocked in-scope journey
failed; any other open failure is `not-ready`; only an otherwise fully green
run is `ready`.
...
6. **Evidence lean and linked** — checkpoints/failures captured, paths resolve, oversized evidence pruned per the layout policy, and the run's `.auth-state/` directory deleted.
...
- **New report file per resume** — an explicit resume continues its existing run-id report; only a new run gets a new file.

...
- **Auth:** the real auth path the persona uses (SSO, magic link). When reuse is necessary, save state only to `<qa-docs-path>/evidence/<run-id>/.auth-state/<persona>.json`, reuse it across sessions *of the same persona only*, never commit it, and delete `.auth-state/` at round close.
...
3. **It survives an appropriate fresh read.** Reload or revisit a stable
   destination URL. For single-use entry links, confirm that reuse is rejected
   and verify the durable outcome through a fresh login or another public read
   path. State that evaporates was never saved.
...
Screenshots go to `<qa-docs-path>/evidence/<run-id>/<charter>-<attempt>-<step>.png`, where `<run-id>` is the report stem `<YYYY-MM-DDTHHMMSSffffffZ>-<nonce8>-<scope>` and `<attempt>` is `baseline`, `retest-1`, or a later numbered retest — checkpoints and failures only, per the evidence policy of the QA docs layout (owned by the `qa-report` skill).
...
In master-jobs, prefix every command below with `rtk pnpm exec`; the short form
keeps the command surface readable while the executable remains project-pinned.
...
agent-browser state save <qa-docs-path>/evidence/<run-id>/.auth-state/<persona>.json
agent-browser state load <qa-docs-path>/evidence/<run-id>/.auth-state/<persona>.json
```

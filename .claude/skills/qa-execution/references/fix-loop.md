...
Anything failing one test → **Decisions for a Human**. When a fix grows beyond bounds mid-edit, restore only the edits introduced by that fix while preserving all pre-existing and concurrent work, restore the finding to `Fail`, and escalate. A half-applied fix is worse than an open bug.
...
When the escalation requires a product/design choice, the matrix row becomes
`Blocked (human decision)` and the scenario's `qa_status` becomes
`blocked-decision`. A large or risky but behaviorally unambiguous engineering
fix remains `Fail` with `fix_status: pending`; its recommendation still appears
here, but the tracker does not mislabel it as a product decision. Both are
terminal for this run and are never silently retried.
...
1. Re-walk the impacted journey **from scratch under its assigned/original persona** — a fresh session (fresh state, real entry), not a resumed browser. A Recovering User may be an additional walker, but never replaces the persona and modality that exposed the failure.
...
3. Only after the original-persona pass: bug → `fixed` and `verified`, matrix row → `Fixed`, tracker row per the schema (`fix_status: fixed`, `retest_status: pass`, `fix_commits` updated).

...
Priority is durable journey metadata, not a cadence-time guess:

- `P0` — release-, security-, privacy-, or data-integrity-critical journey.
- `P1` — core, frequent, recovery, or commercially important journey.
- `P2` — supporting journey whose failure does not block the core product.

Full QA includes every `P0` and `P1` journey. Update priority when the product
risk changes, and record the reason in the journey history.

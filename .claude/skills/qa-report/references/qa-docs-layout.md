...
├── reports/
│   └── <UTC-microseconds>-<nonce8>-<scope>.md # one collision-resistant id per run
├── evidence/
│   └── <UTC-microseconds>-<nonce8>-<scope>/ # screenshots/auth state — gitignored
├── automation-backlog/
...
| ----------------------------------------------------- | -------------------------------------------- |
| `scenarios/` files and their lifecycle fields         | `reports/<UTC-run-id>-<scope>.md`            |
| `bugs/` (ids stable forever)                          | `evidence/<UTC-run-id>-<scope>/`             |
...
2. Seed the four templates in `templates/` (scenario + bug + charter from `qa-report/assets/`, report from the sibling skill's `qa-execution/assets/report-template.md` when installed). If project policy selects canonical sharing, create relative symlinks to those assets; copy only when the project explicitly wants independently customized templates.
...
4. **Explode a legacy tracker into scenario files.** Run `rtk python3 <qa-report-skill-dir>/scripts/explode_state.py <csv-path> <qa-docs-path>` — a bootstrap helper: reads the CSV, writes one `scenarios/<id>.md` per row (grandfathered id = filename, packed notes unfolded into body paragraphs), and touches nothing else. Validate the round trip with `rtk python3 <qa-report-skill-dir>/scripts/materialize_state.py <qa-docs-path>`; it must parse every produced file and report exactly the original row count. Then untrack the generated artifacts that stay behind (`rtk git rm --cached <qa-docs-path>/state.csv`, evidence dirs) in the same adoption commit, so the gitignore block takes effect.

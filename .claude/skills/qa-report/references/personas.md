...
Define each project persona in `<qa-docs-path>/personas.md` with this full
schema. Charters copy the persona's `name`, `device`, `network`, and `locale`;
bug files reference its exact `name` in `Persona Affected:`.
...
- **Partial-persona full cycles** — a full cycle covers every project persona across its sessions, or user segments go unverified.
- **Mixing personas mid-charter** — one charter has one persona; run it to completion before starting another charter.

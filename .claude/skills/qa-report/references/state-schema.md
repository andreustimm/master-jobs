...
- Every field is an enum, an id, a path, or a single sentence with a fixed role; lists are semicolon-separated. **All free-form or multiline notes live in the body.** This is what keeps the tracker queryable — "all failing scenarios" must be one search (`rtk rg -l 'qa_status: fail' scenarios/`), not an interpretation exercise.
...
```bash
rtk python3 <qa-report-skill-dir>/scripts/materialize_state.py <qa-docs-path>

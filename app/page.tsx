import Link from "next/link";
import { clusterBreakdown, corpusStats, listBoard, pipelineCounts } from "../src/core/db/repo.ts";
import { Fit, Legend, ScoreBar, Stat, Chip } from "./ui";

export const dynamic = "force-dynamic";

export default async function Cockpit() {
  const [stats, counts, clusters, top] = await Promise.all([
    corpusStats(),
    pipelineCounts(),
    clusterBreakdown(45),
    listBoard({ minFit: 60, limit: 12 }),
  ]);

  const tracked = Object.values(counts).reduce((a, b) => a + b, 0);

  return (
    <main>
      <header style={{ padding: "48px 0 28px" }}>
        <p
          className="mono"
          style={{ fontSize: 11.5, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--text-3)", margin: "0 0 12px" }}
        >
          Cockpit
        </p>
        <h1 style={{ fontSize: 38, fontWeight: 700, letterSpacing: "-.02em", margin: "0 0 10px" }}>
          O que vale seu tempo hoje
        </h1>
        <p style={{ color: "var(--text-2)", maxWidth: "62ch", margin: 0 }}>
          Ranqueamento determinístico contra o seu perfil. Cada barra mostra{" "}
          <strong>de onde veio a nota</strong> — uma nota alta sustentada só por
          elegibilidade e salário costuma ser falso positivo.
        </p>
      </header>

      <div
        style={{
          display: "grid",
          gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))",
          gap: 1,
          background: "var(--line)",
          border: "1px solid var(--line)",
          borderRadius: 10,
          overflow: "hidden",
          marginBottom: 40,
        }}
      >
        <Stat value={stats?.open?.toLocaleString("pt-BR") ?? "0"} label="vagas abertas" />
        <Stat value={stats?.companies?.toLocaleString("pt-BR") ?? "0"} label="empresas" />
        <Stat value={stats?.above45 ?? 0} label="acima de 45" accent />
        <Stat value={stats?.above60 ?? 0} label="acima de 60" accent />
        <Stat value={stats?.above70 ?? 0} label="acima de 70" accent />
        <Stat value={Number(stats?.best ?? 0).toFixed(0)} label="melhor fit" />
        <Stat value={tracked} label="no funil" />
      </div>

      <section>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 6 }}>
          <h2 style={{ fontSize: 22, fontWeight: 600, margin: 0 }}>Topo do ranking</h2>
          <Link href="/jobs" style={{ fontSize: 13, color: "var(--accent)" }}>
            ver todas →
          </Link>
        </div>
        <p style={{ color: "var(--text-3)", fontSize: 13.5, margin: "0 0 18px" }}>
          Acima de 60 de aderência.
        </p>
        <div style={{ marginBottom: 16 }}>
          <Legend />
        </div>
        <JobList rows={top} />
      </section>

      {clusters.length > 0 && (
        <section style={{ marginTop: 48 }}>
          <h2 style={{ fontSize: 22, fontWeight: 600, margin: "0 0 4px" }}>Por cluster</h2>
          <p style={{ color: "var(--text-3)", fontSize: 13.5, margin: "0 0 18px" }}>
            Acima do corte de 45. Se <code>other</code> domina, o corte está frouxo.
          </p>
          <div style={{ display: "grid", gap: 10, maxWidth: 560 }}>
            {clusters.map((c) => {
              const max = Math.max(...clusters.map((x) => Number(x.n)));
              return (
                <div key={c.cluster}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                    <span className="mono">{c.cluster}</span>
                    <span className="mono" style={{ color: "var(--text-2)" }}>
                      {c.n} · melhor {Number(c.best).toFixed(0)}
                    </span>
                  </div>
                  <div style={{ height: 4, background: "var(--line)", borderRadius: 2, marginTop: 5 }}>
                    <span
                      style={{
                        display: "block",
                        height: "100%",
                        width: `${(Number(c.n) / max) * 100}%`,
                        background: "var(--accent)",
                        borderRadius: 2,
                      }}
                    />
                  </div>
                </div>
              );
            })}
          </div>
        </section>
      )}
    </main>
  );
}

type Row = Awaited<ReturnType<typeof listBoard>>[number];

export function JobList({ rows }: { rows: Row[] }) {
  if (rows.length === 0) {
    return (
      <p style={{ color: "var(--text-3)", padding: "24px 0" }}>
        Nenhuma vaga acima do corte. Rode <code className="mono">pnpm jho jobs sync</code>.
      </p>
    );
  }
  return (
    <div
      style={{
        display: "flex",
        flexDirection: "column",
        gap: 1,
        background: "var(--line)",
        border: "1px solid var(--line)",
        borderRadius: 10,
        overflow: "hidden",
      }}
    >
      {rows.map((r) => {
        const blockers = Array.isArray(r.blockers) ? (r.blockers as string[]) : [];
        return (
          <article
            key={r.jobId}
            style={{
              background: "var(--surface)",
              display: "grid",
              gridTemplateColumns: "64px 1fr auto",
              gap: 16,
              alignItems: "start",
              padding: "14px 18px",
            }}
          >
            <div style={{ textAlign: "center", paddingTop: 2 }}>
              <Fit value={r.fit} />
            </div>
            <div style={{ minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap" }}>
                <Link
                  href={`/jobs/${r.jobId}`}
                  style={{ fontSize: 15.5, fontWeight: 600, color: "var(--text)", textDecoration: "none" }}
                >
                  {r.title}
                </Link>
                {r.status && <Chip>{r.status}</Chip>}
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 12px", marginTop: 4, fontSize: 12.5, color: "var(--text-2)" }}>
                <span style={{ fontWeight: 600, color: "var(--text)" }}>{r.companyName}</span>
                {r.cluster && (
                  <span className="mono" style={{ fontSize: 11, color: "var(--accent)" }}>
                    {r.cluster}
                  </span>
                )}
                {r.locationRaw && <span>{r.locationRaw.slice(0, 70)}</span>}
              </div>
              <div style={{ marginTop: 9 }}>
                <ScoreBar parts={r as unknown as Record<string, number | null>} />
              </div>
              {blockers.length > 0 && (
                <div style={{ marginTop: 8, fontSize: 12, color: "var(--color-alert)" }}>
                  ⚠ {blockers.join("; ")}
                </div>
              )}
            </div>
            <a
              className="mono"
              href={r.applyUrl ?? r.url}
              target="_blank"
              rel="noopener"
              style={{ fontSize: 12, color: "var(--accent)", whiteSpace: "nowrap", paddingTop: 5 }}
            >
              aplicar →
            </a>
          </article>
        );
      })}
    </div>
  );
}

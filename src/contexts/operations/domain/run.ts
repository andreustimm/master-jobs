/**
 * Execuções de captura e de verificação: estados, chave de idempotência,
 * lease, composição do pai e redação do erro.
 *
 * Puro — sem banco, sem rede, sem relógio. O instante entra como argumento.
 * A tabela `source_run` guarda o que estas funções decidem; a infra só aplica.
 */
...
export const RUN_STATUSES = ["queued", "running", "succeeded", "partial", "failed", "cancelled", "interrupted"] as const;
...
/**
 * O estado do pai a partir das filhas terminais. Uma filha que falhou não
 * esconde as outras: o pai fica `partial` e cada filha mostra o seu.
 */
...
/**
 * A revisão que entra na chave de um escopo com várias fontes: o conjunto de
 * `(fonte, revisão)`, não a maior revisão — editar uma fonte de revisão menor
 * que a de outra também muda o pedido, e a chave precisa mudar junto.
 */
 ...
 * colide com um "Buscar agora" da mesma fonte, que é outro pedido — e rodar a
 * mesma fonte duas vezes é idempotente (observação por `fingerprint`).
 */
...
/**
 * `running` sem batimento além do lease morreu com o processo que a tocava.
 * Execução terminal nunca vence; `queued` também não — ela espera executor, e
 * o motivo da espera fica visível na linha.
 */
export function isStale(run: { status: RunStatus; heartbeatAt: string | null }, now: string, leaseMs: number): boolean {
  if (run.status !== "running") return false;
  if (run.heartbeatAt === null) return true;
  return Date.parse(now) - Date.parse(run.heartbeatAt) > leaseMs;
}
...
// Sequência de 8+ dígitos com separadores comuns de telefone.
